    ...
    while True:
        ...
        if s != "":
            if int(s) == 0:
                ...
                if isRevoke.lower() == "yes":
                    await client.delete_messages(chats[int(dialog)].id, _messages[int(message)].id, revoke=True)
                else:
                    await client.delete_messages(chats[int(dialog)].id, _messages[int(message)].id, revoke=False)
